...
**🔧 Automatically sync contract addresses** in `ponder/ponder.config.ts` with the latest deployment addresses.

Run the helper script:

```bash
node scripts/updatePonderConfig.js
```

The script reads `scripts/deployment.json` (generated by the deployment step) and updates the `address` fields in `ponder/ponder.config.ts` accordingly. Run this after each fresh deployment.
