...
// 1. Define __dirname at the top level only
const __dirname = path.dirname(fileURLToPath(import.meta.url));

async function main() {
    const [deployer, agent, reviewer, juror] = await hre.ethers.getSigners();
    console.log("Using pre‑deployed contracts with the account:", deployer.address);

    // 2. Removed duplicate imports and declaration here. 
    // Directly use the top-level __dirname
    ...
    // Ensure the agent connects to submit their own task
    ...
    // Note: In Ethers v6 balances are BigInts, so subtraction works natively.
    ...
    // 3. FIXED: Ethers v6 Event Parsing
    // receipt.events is deprecated. We must iterate logs and parse them against the interface.
    let disputeId = null;
    if (receipt.logs) {
        for (const log of receipt.logs) {
            try {
                // Attempt to parse the log using the contract interface
                const parsedLog = juryRegistry.interface.parseLog(log);
                if (parsedLog && parsedLog.name === "DisputeCreated") {
                    disputeId = parsedLog.args.disputeId;
                    break;
                }
            } catch (e) {
                // Ignore logs that don't match the interface (e.g., internal ERC20 transfers)
            }
        }
    }

    // Fallback if event parsing failed (only for demo purposes)
    if (!disputeId) {
        console.warn("Could not parse dispute ID from logs, assuming ID 1");
        disputeId = 1;
    }
